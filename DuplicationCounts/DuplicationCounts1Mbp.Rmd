...
pi655_filtered <- chromosome %>% group_by(CHROM) %>% filter(X655972 != '0/0')
pi510_filtered <- chromosome %>% group_by(CHROM) %>% filter(X510757 != '0/0')
pi506_filtered <- chromosome %>% group_by(CHROM) %>% filter(X506069 != '0/0')
pi329_filtered <- chromosome %>% group_by(CHROM) %>% filter(X329311 != '0/0')    
pi300_filtered <- chromosome %>% group_by(CHROM) %>% filter(X300119 != '0/0')     
...
pi300 = pi300_filtered %>% count(interval) %>% rename(pi300119 = n)
...
```{r}
merges <- full_join(CA, pi655, by = "interval")
merges <- full_join(merges, pi510, by = "interval")
merges <- full_join(pi506, merges, by = "interval")
merges <- full_join(pi329, merges, by = "interval")
#merges <- full_join(pi300, merges, by = "interval") commented out because the grassl line (300119) is neither a sweet nor biomass variety
merges <- full_join(leoti, merges, by = "interval")
merges <- full_join(pi229, merges, by = "interval")
merges <- full_join(pi297, merges, by = "interval")
merges <- full_join(rio, merges, by = "interval")
