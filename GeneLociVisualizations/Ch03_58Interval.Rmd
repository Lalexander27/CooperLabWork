...
if(access =="Louie") {
  setwd("C:\\Users\\louie\\OneDrive\\Documents\\ResearchFiles\\DuplicationCounts")
}
...
```{r}
chromosome <- duplications %>% filter(CHROM == "Chr03") %>% select(-c(ID, REF, ALT, QUAL, FILTER, INFO))

#This splits the chromosome into "steps" of 1M bp in size
chromosome$interval <- floor(chromosome$POS/10000)
chromosome <- chromosome %>% filter(POS >= 58000000 & POS <= 59000000)
```
...
#merges <- full_join(pi300, merges, by = "interval") commented out because this line (300119) is neither a sweet nor biomass variety
...
for (inv in interval){
  if (!(inv %in% merges$interval)){
    ...
    merges[nrow(merges), "CHROM"] = "Chr03"
  }
}
...
avg_Distance <- avg_Distance %>% mutate(distance = abs(sweetAVG - cellulosicAVG)) %>% select(-CHROM)
avg_Distance <- avg_Distance[,2:14]
...
data_long <- data_long %>% filter(variable == "cellulosicAVG" | variable == "sweetAVG") %>% mutate(color = ifelse(variable == "cellulosicAVG", "blue", "orange"))
...
ggplot(data = data_long) + geom_line(data=splineSw, aes(x=x, y=y), color = "#E69F00") + geom_line(data=splineCe, aes(x=x, y=y), color= "#009E73") + geom_area(data=splineSw, aes(x=x, y=y), fill = "#E69F00") + geom_area(data=splineCe, aes(x=x, y=y), fill = "#009E73") +
  annotate("rect", xmin = 5893, xmax = 5897, ymin = -1.2, ymax = 2.0, alpha = .25,fill = "#921414") + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.background = element_blank(), axis.line = element_line(colour = "black")) +   ggtitle("Chromosome 3 Interval 58 Frequency of Duplications") + xlab("Interval (10,000 bp)") + ylab("Number of Features")
```
